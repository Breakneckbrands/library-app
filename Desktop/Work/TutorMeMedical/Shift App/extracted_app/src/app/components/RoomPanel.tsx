            ...
            {showQuickAdd && (
              <div className="px-3 pb-3 pt-2 border-t border-gray-100 bg-gray-50" onClick={e => e.stopPropagation()}>
                ...
                <div className="flex flex-col gap-2">
                  ...
                  />
                  <div className="flex items-center gap-2">
                    <label className="flex items-center gap-1.5 flex-1 border border-indigo-200 rounded-lg px-2.5 py-2 bg-indigo-50 cursor-pointer">
                      <Clock className="w-3.5 h-3.5 text-indigo-400 shrink-0" />
                      <span className="text-[11px] font-bold text-indigo-400 uppercase tracking-wide shrink-0">Time</span>
                      <input
                        type="time"
                        value={quickAddTime}
                        onChange={e => setQuickAddTime(e.target.value)}
                        onClick={e => { if (!quickAddTime) setQuickAddTime(nowTimeStr()); e.stopPropagation(); }}
                        className="flex-1 bg-transparent text-sm text-indigo-700 focus:outline-none min-w-0"
                      />
                    </label>
